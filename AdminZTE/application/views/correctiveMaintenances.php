...
<!-- MODAL MP -->
    ...
    <script data-config>
          $(function () {
          var ticket = <?php echo json_encode($tickets); ?>;
console.log(ticket);
          ...
          for(var i = 0; i < ticket.length ; i++){
            if(ticket[i].idMaintenance){
              console.log(ticket[i].idMaintenance);

              var año = ticket[i].dateStart.split("-")[0];
              var mes = ticket[i].dateFinish.split("-")[1];
              var linea = "<tr>";
              //linea = linea  + "<td><div style='width: 20px; height: 20px; border-radius: 50%; background: #"+ticket[i].maintenance[j].ticket[0].color+"; '>&nbsp;</div></td>";
              linea = linea  + "<td>"+ticket[i].idMaintenance.region.split(" ")[1]+"</td>";
              linea = linea  + "<td>"+ticket[i].idMaintenance.deparment+" / "+ticket[i].idMaintenance.city+"</td>";
              linea = linea  + "<td>"+ticket[i].id+"</td></tr>";
